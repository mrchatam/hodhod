...
// ListUserCreateTemplates loads panel user create templates from KV.
func ListUserCreateTemplates(ctx context.Context, store *db.Store, panelID int64) ([]UserCreateTemplate, error) {
...
// SaveUserCreateTemplate upserts a panel user create template.
func SaveUserCreateTemplate(ctx context.Context, store *db.Store, panelID int64, tpl UserCreateTemplate) error {
	...
	list, _ := ListUserCreateTemplates(ctx, store, panelID)
	found := false
	for i, t := range list {
...
// DeleteUserCreateTemplate removes a template by name.
func DeleteUserCreateTemplate(ctx context.Context, store *db.Store, panelID int64, name string) error {
	list, err := ListUserCreateTemplates(ctx, store, panelID)
	if err != nil {
		return err
	}
...
// FindUserCreateTemplate returns a template by name.
