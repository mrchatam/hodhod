...
echo "=== Recommended Hodhod fixes (pick one) ==="
echo ""
echo "A) Native mode (simplest — app uses host network like curl):"
echo "   DEPLOY_MODE=native in .env, then: bash install.sh → Update"
echo ""
echo "B) Host-network Docker app (keep app in Docker):"
echo "   HODHOD_HOST_NETWORK=1"
echo "   HODHOD_DB_HOST_PORT=15432"
echo "   HTTP_PORT must match Nginx (e.g. 9000)"
echo "   bash install.sh → Update"
echo ""
echo "C) Fix Docker SNAT on the server (requires root, varies by VPN/firewall setup)"
echo "   After fixing, verify: docker run --rm curlimages/curl:8.5.0 -sS --max-time 12 https://api.telegram.org/"
